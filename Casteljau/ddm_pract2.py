...
def ControlMesh(n, length): 
	...
	#Handle corner Case leading to division by 0:
	if(n == 1):
	...
	for y in range (0, n):
		for x in range (0, n):
			...
			vertices.append((x*step, y*step, z))  
...
def CreateFacesFromMesh(n):
	...
	for y in range (0, n - 1):
		for x in range (0, n - 1):
			...
			leftTopcorner = x + y * n + 1
			leftBotcorner  = x + y * n
			rightBotcorner = x + n + y * n
...
def DeCasteljau(A, n, s):
	...
	for i in range(0,n):
		...
		column = []
		for j in range(0, n):
			 column.append(mathutils.Vector(A[i+j*n]))
